...
st.title("📊 四次模考成績分析系統")
st.caption("上傳四次模考的 Excel 原始檔，系統將自動產生成績分析 CSV 與學生個人成績分析單。")
...
# 根據分析，第四次模考的格式與第一次完全相同
def parse_exam4(file_obj, prefix):
    return parse_exam1(file_obj, prefix)


def build_merged_df(f1, f2, f3, f4):
    parsers = [(f1, '一模', parse_exam1), (f2, '二模', parse_exam2), (f3, '三模', parse_exam3), (f4, '四模', parse_exam4)]
    merged = None
    ...
    for subj in subjects:
        if f'四模_{subj}' in merged.columns and f'三模_{subj}' in merged.columns:
            merged[f'近期{subj}變化'] = merged[f'四模_{subj}'] - merged[f'三模_{subj}']
        if f'四模_{subj}' in merged.columns and f'一模_{subj}' in merged.columns:
            merged[f'總體{subj}成長'] = merged[f'四模_{subj}'] - merged[f'一模_{subj}']
    for r in ['班排', '校排', '區排']:
        if f'四模_{r}' in merged.columns and f'三模_{r}' in merged.columns:
            merged[f'近期{r}進步'] = merged[f'三模_{r}'] - merged[f'四模_{r}']
        if f'四模_{r}' in merged.columns and f'一模_{r}' in merged.columns:
            merged[f'總體{r}進步'] = merged[f'一模_{r}'] - merged[f'四模_{r}']
    return merged
...
def generate_card(row, class_name="922班"):
    ...
    for subj in SUBJECTS:
        ...
        score_rows += f'<tr><td class="label">{subj}</td><td>{fmt(vals[0])}</td><td>{fmt(vals[1])}</td><td>{fmt(vals[2])}</td><td>{fmt(vals[3])}</td><td>{arrow(row.get(f"近期{subj}變化", np.nan))}</td><td>{arrow(row.get(f"總體{subj}成長", np.nan))}</td></tr>'

    t_vals = [row.get(f'{e}_總分', np.nan) for e in EXAMS]
    total_row = f'<tr class="total-row"><td class="label">總分(含寫作)</td><td>{fmt(t_vals[0])}</td><td>{fmt(t_vals[1])}</td><td>{fmt(t_vals[2])}</td><td>{fmt(t_vals[3])}</td><td>{arrow(row.get("近期總分變化", np.nan))}</td><td>{arrow(row.get("總體總分成長", np.nan))}</td></tr>'

    w_vals = [row.get(f'{e}_寫作', np.nan) for e in EXAMS]
    writing_row = f'<tr><td class="label">寫作</td><td>{fmt(w_vals[0])}</td><td>{fmt(w_vals[1])}</td><td>{fmt(w_vals[2])}</td><td>{fmt(w_vals[3])}</td><td>{arrow(row.get("近期寫作變化", np.nan))}</td><td>{arrow(row.get("總體寫作成長", np.nan))}</td></tr>'

    colors = ['#4e6ef2', '#e05c5c', '#27ae60', '#e67e22', '#8e44ad']
    subj_charts = ''
    for subj, color in zip(SUBJECTS, colors):
        vals = [row.get(f'{e}_{subj}', np.nan) for e in EXAMS]
        subj_charts += f'<div class="chart-item"><div class="chart-title" style="color:{color}">{subj}</div>{make_svg(vals, ["一模","二模","三模","四模"], color)}</div>'

    t_chart = make_svg(t_vals, ['一模', '二模', '三模', '四模'], '#1a1a2e', width=300, height=110)
    ...
    for r in RANKS:
        ...
        rank_rows += f'<tr><td class="label">{RANK_LABELS[r]}</td><td>{fmt(r_vals[0],0)}</td><td>{fmt(r_vals[1],0)}</td><td>{fmt(r_vals[2],0)}</td><td>{fmt(r_vals[3],0)}</td><td>{rank_arrow(row.get(f"近期{r}進步", np.nan))}</td><td>{rank_arrow(row.get(f"總體{r}進步", np.nan))}</td></tr>'
        rank_charts += f'<div class="chart-item"><div class="chart-title" style="color:{rcc[r]}">{RANK_LABELS[r]}</div>{make_svg(r_vals, ["一模","二模","三模","四模"], rcc[r], lower_is_better=True)}</div>'

    return f"""<div class="card">
  <div class="header">
    <div class="header-left"><h1>四次模考個人成績分析單</h1><div class="subtitle">{class_name} ｜ 分析日期：{today}</div></div>
    <div class="header-badge"><div class="seat">座號 {sid}</div><div class="name">{name}</div></div>
  </div>
  <div class="section-title">📊 各科成績比較</div>
  <table><thead><tr><th style="text-align:left;width:110px">科目</th><th>第一次模考</th><th>第二次模考</th><th>第三次模考</th><th>第四次模考</th><th>近期變化<br><small style="font-weight:400;color:#888">(四模－三模)</small></th><th>整體成長<br><small style="font-weight:400;color:#888">(四模－一模)</small></th></tr></thead>
  ...
  <table><thead><tr><th style="text-align:left;width:110px">排名類型</th><th>第一次模考</th><th>第二次模考</th><th>第三次模考</th><th>第四次模考</th><th>近期進退步<br><small style="font-weight:400;color:#888">(三模排－四模排)</small></th><th>整體進退步<br><small style="font-weight:400;color:#888">(一模排－四模排)</small></th></tr></thead>
...
if run_btn:
    with st.spinner("🔄 正在讀取並分析資料…"):
        try:
            merged = build_merged_df(file1, file2, file3, file4)
    ...
    if '四模_總分' in merged.columns and '一模_總分' in merged.columns:
        avg_growth = (merged['四模_總分'] - merged['一模_總分']).mean()
        col2.metric("平均總分成長（四模－一模）", f"{avg_growth:+.2f}")
    if '四模_班排' in merged.columns and '一模_班排' in merged.columns:
        avg_rank = (merged['一模_班排'] - merged['四模_班排']).mean()
    ...
    with st.spinner("📄 正在產生學生成績分析單…"):
        ...
        with zipfile.ZipFile(zip_buf, 'w', zipfile.ZIP_DEFLATED) as zf:
            for _, row in merged.iterrows():
                ...
                cards.append(card)
                individual_html = wrap_html(card, f"{sid} {name} 四次模考成績分析單")
    ...
    with dl1:
        st.download_button(
            label="📥 下載 CSV 成績表",
            data=csv_bytes,
            file_name=f"{class_name}_四次模考綜合分析表.csv",
            mime="text/csv",
...
elif not (file1 and file2 and file3 and file4):
    st.info("👈 請在左側上傳四次模考 Excel 檔案，再按「執行分析」。")